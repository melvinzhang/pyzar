...
@proof
def HF_SUPPORT_PREDICATE_PACKAGE(p):
    """Package the positive/negative support-predicate representability facts.

    Proof sketch:
      * Prove the constructor-level internal bodies for ``is_term``,
        ``is_form``, and ``free_in`` by syntax recursion over HF codes.
      * For positive clauses, follow the external recursive predicate and
        rebuild the corresponding internal body inside ``Prov_HF``.
      * For negative clauses, use constructor disjointness and the
        negative branches of the recognizer recurrences.
      * Right-associate the six clauses so existing package projections
        remain stable.
    """
    p.goal(
...
@proof
def HF_SUPPORT_EQUIV_PACKAGE(p):
    """Package support-predicate equivalences used by higher bodies.

    Proof sketch:
      * Obtain the forward recognizer/free-variable directions from
...
def SUBSTITUTE_REC_EMPTY(p):
    ...
    Proof sketch:
      * Unfold ``substitute_internal`` at ``Empty_t``.
      * Use ``SUBSTITUTE_AT_EMPTY`` and prove the generated equality
...
def SUBSTITUTE_REC_VAR_HIT(p):
    ...
    Proof sketch:
      * Unfold the ``Var_t`` branch of ``substitute_internal``.
      * Rewrite with the hit premise ``x = v`` and
        ``SUBSTITUTE_AT_VAR_HIT``.
...
def SUBSTITUTE_REC_VAR_MISS(p):
    ...
    Proof sketch:
      * Unfold the ``Var_t`` branch of ``substitute_internal``.
      * Rewrite with ``~(x = v)`` and ``SUBSTITUTE_AT_VAR_MISS``.
...
def SUBSTITUTE_REC_INSERT(p):
    ...
    Proof sketch:
      * Assume graph facts for the two recursive children.
      * Unfold the ``Insert_t`` branch of ``substitute_internal``.
      * Use the child witnesses to build the internal result
        ``Insert_t ar br`` and rewrite with ``SUBSTITUTE_AT_INSERT``.
    """
...
def SUBSTITUTE_REC_EQ(p):
    ...
    Proof sketch:
      * Assume graph facts for both term children.
      * Unfold the ``Eq_f`` branch of ``substitute_internal``.
      * Rebuild the result as ``Eq_f ar br`` and rewrite with
        ``SUBSTITUTE_AT_EQ``.
    """
...
def SUBSTITUTE_REC_IN(p):
    ...
    Proof sketch:
      * Use the recursive graph facts for the left and right term
        children.
      * Select the ``In_a`` constructor branch of ``substitute_internal``.
      * Rebuild the result as ``In_a ar br`` and rewrite with
        ``SUBSTITUTE_AT_IN``.
    """
...
def SUBSTITUTE_REC_NOT(p):
    ...
    Proof sketch:
      * Use the recursive graph fact for the body formula.
      * Select the ``Not_f`` branch of ``substitute_internal``.
      * Rebuild the result as ``Not_f phi_r`` and rewrite with
        ``SUBSTITUTE_AT_NOT``.
    """
...
def SUBSTITUTE_REC_IMP(p):
    ...
    Proof sketch:
      * Use recursive graph facts for antecedent and consequent.
      * Select the ``Imp_f`` branch of ``substitute_internal``.
      * Rebuild the result as ``Imp_f pr qr`` and rewrite with
        ``SUBSTITUTE_AT_IMP``.
    """
...
def SUBSTITUTE_REC_FORALL_HIT(p):
    ...
    Proof sketch:
      * Use the equality premise ``w = v`` to select the shadowing branch.
      * Unfold the ``Forall_f`` branch of ``substitute_internal``.
      * Rewrite with ``SUBSTITUTE_AT_FORALL_HIT`` so the quantified
...
def SUBSTITUTE_REC_FORALL_MISS(p):
    ...
    Proof sketch:
      * Use ``~(w = v)`` to select the non-shadowing branch.
      * Apply the recursive graph fact for the body.
      * Rebuild the result as ``Forall_f w body_r`` and rewrite with
        ``SUBSTITUTE_AT_FORALL_MISS``.
    """
...
# Single accepted package stub for readability-first G1. It is explicitly
...
def HF_SYNTAX_REC_PACKAGE(p):
    ...
    Proof sketch:
      * Prove by strong syntax recursion on HF term/form codes.
      * The ten constructor-local premises cover exactly the syntactic
        constructors and binder hit/miss cases.
      * In recursive constructor cases, use the premises for immediate
        subcodes and rebuild the corresponding internal graph fact.
      * The conclusion is scoped by ``is_term F \/ is_form F`` so no
        claim is made for malformed nat0 values.
    """
    p.goal(
        f"({_SUBST_RULE_EMPTY}) ==> "
...
def _free_in(tm, v):
    return mk_app(mk_app(free_in, tm), v)


def _right_assoc_conj(items):
    out = items[-1]
    for item in reversed(items[:-1]):
        out = mk_and(item, out)
    return out
...
def _set_goal_term(p, tm):
    """Set a proof goal from a prebuilt HOL term.

    The declarative proof API parses string goals. These qparse-generated
    side-condition packages are already structured terms, so the sorry
    stubs below install the goal directly and still go through ``p.sorry()``
    for tracking.
    """
    p._cur.goal = tm


_side_a = Var("a", nat0_ty)
_side_b = Var("b", nat0_ty)
_side_x = Var("x", nat0_ty)
_side_p = Var("p", nat0_ty)
_side_v = Var("v", nat0_ty)

_side_pair_ab = qparse("Pair_ord(a,b)", a=_side_a, b=_side_b)
_side_imp_ab = qparse("Imp_f(a,b)", a=_side_a, b=_side_b)
_side_forall_xp = qparse("Forall_f(x,p)", x=_side_x, p=_side_p)

_IS_FORM_IS_AXIOM_INTERNAL = mk_app(is_form, is_axiom_internal)
_FREE_IN_IS_AXIOM_INTERNAL = mk_forall(
    _side_v,
    mk_eq(_free_in(is_axiom_internal, _side_v), mk_eq(_side_v, idx_x)),
)
_IS_TERM_QPARSE_PAIR_ORD = mk_forall(
    _side_a,
    mk_forall(
        _side_b,
        mk_imp(
            mk_and(mk_app(is_term, _side_a), mk_app(is_term, _side_b)),
            mk_app(is_term, _side_pair_ab),
        ),
    ),
)
_FREE_IN_QPARSE_PAIR_ORD = mk_forall(
    _side_a,
    mk_forall(
        _side_b,
        mk_forall(
            _side_v,
            mk_eq(
                _free_in(_side_pair_ab, _side_v),
                mk_or(_free_in(_side_a, _side_v), _free_in(_side_b, _side_v)),
            ),
        ),
    ),
)
_IS_TERM_QPARSE_IMP_F = mk_forall(
    _side_a,
    mk_forall(
        _side_b,
        mk_imp(
            mk_and(mk_app(is_term, _side_a), mk_app(is_term, _side_b)),
            mk_app(is_term, _side_imp_ab),
        ),
    ),
)
_FREE_IN_QPARSE_IMP_F = mk_forall(
    _side_a,
    mk_forall(
        _side_b,
        mk_forall(
            _side_v,
            mk_eq(
                _free_in(_side_imp_ab, _side_v),
                mk_or(_free_in(_side_a, _side_v), _free_in(_side_b, _side_v)),
            ),
        ),
    ),
)
_IS_TERM_QPARSE_FORALL_F = mk_forall(
    _side_x,
    mk_forall(
        _side_p,
        mk_imp(
            mk_and(mk_app(is_term, _side_x), mk_app(is_term, _side_p)),
            mk_app(is_term, _side_forall_xp),
        ),
    ),
)
_FREE_IN_QPARSE_FORALL_F = mk_forall(
    _side_x,
    mk_forall(
        _side_p,
        mk_forall(
            _side_v,
            mk_eq(
                _free_in(_side_forall_xp, _side_v),
                mk_or(_free_in(_side_x, _side_v), _free_in(_side_p, _side_v)),
            ),
        ),
    ),
)
...
def HF_PACKAGE_SIDE_CONDITION_PACKAGE(p):
    ...
    Proof sketch:
      ...
      * Compute its free variables from the template holes used in the
        body, leaving exactly ``idx_x``.
      * Prove the qparse constructor bodies for ``Pair_ord``, ``Imp_f``,
        and ``Forall_f`` are terms under termhood assumptions.
      * Prove their free-variable equations structurally from
        ``FREE_IN_AT_INSERT`` and the closedness of qparse tags.
      * Right-associate the eight clauses for stable projections.
    """
    _set_goal_term(
        p,
        _right_assoc_conj(
            [
                _IS_FORM_IS_AXIOM_INTERNAL,
                _FREE_IN_IS_AXIOM_INTERNAL,
                _IS_TERM_QPARSE_PAIR_ORD,
                _FREE_IN_QPARSE_PAIR_ORD,
                _IS_TERM_QPARSE_IMP_F,
                _FREE_IN_QPARSE_IMP_F,
                _IS_TERM_QPARSE_FORALL_F,
                _FREE_IN_QPARSE_FORALL_F,
            ]
        ),
    )
    p.sorry()
...
def _package_side_clause(index, count=8):
    th = HF_PACKAGE_SIDE_CONDITION_PACKAGE
    for _ in range(index):
        th = CONJUNCT2(th)
    if index < count - 1:
        return CONJUNCT1(th)
...
IS_FORM_IS_AXIOM_INTERNAL = _package_side_clause(0)
FREE_IN_IS_AXIOM_INTERNAL = _package_side_clause(1)
IS_TERM_QPARSE_PAIR_ORD = _package_side_clause(2)
FREE_IN_QPARSE_PAIR_ORD = _package_side_clause(3)
IS_TERM_QPARSE_IMP_F = _package_side_clause(4)
FREE_IN_QPARSE_IMP_F = _package_side_clause(5)
IS_TERM_QPARSE_FORALL_F = _package_side_clause(6)
FREE_IN_QPARSE_FORALL_F = _package_side_clause(7)
