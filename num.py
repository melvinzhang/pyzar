...
def _IND_SUC_PROPS_PAIR(prf):
    ...
    prf.thus("ONE_ONE_ind IND_SUC /\\ ~(ONTO_ind IND_SUC)").by_rewrite_of(
        "f_eq", [SYM(IND_SUC_DEF)]
    )
...
def _EXISTS_WITNESS(prf):
    ...
    prf.have("not_forall: ~(!y:ind. ?x:ind. y = IND_SUC x)").by_eq_mp(
        _NOT_ONTO_UNFOLD, NOT_ONTO_IND_SUC
    )
    # NOT_FORALL_TO_EX_NOT swaps the outer quantifier to existential.
    _outer_pred = parse("\\y:ind. ?x:ind. y = IND_SUC x")
    prf.have("ex_not: ?y:ind. ~(?x:ind. y = IND_SUC x)").by_thm(
        NOT_FORALL_TO_EX_NOT(prf.fact("not_forall"), _outer_pred)
    )
    ...
    prf.have("forall_neq: !x:ind. ~(y = IND_SUC x)").by_thm(
        NOT_EX_TO_FORALL_NOT(prf.fact("y_eq"), _inner_pred)
    )
    with prf.have("forall_swapped: !x:ind. ~(IND_SUC x = y)").proof():
        prf.fix("x")
        prf.thus("~(IND_SUC x = y)").by_thm(
            NE_SYM(SPEC(prf._parse("x"), prf.fact("forall_neq")))
        )
    prf.thus("?z:ind. !x:ind. ~(IND_SUC x = z)").by_witness("y", "forall_swapped")


# Define IND_1 = @z. !x. ~(IND_SUC x = z).
IND_1_DEF = _define("IND_1", ind_ty, "@z:ind. !x:ind. ~(IND_SUC x = z)")
...
def IND_SUC_NEQ_IND_1(prf):
    ...
    prf.thus("!x:ind. ~(IND_SUC x = IND_1)").by_rewrite_of("z_eq", [SYM(IND_1_DEF)])
...
NUM_REP_DEF = _define(
    "NUM_REP",
    _P_ind_ty,
    "\\a:ind. !P:ind->bool. P IND_1 /\\ (!i:ind. P i ==> P (IND_SUC i)) ==> P a",
)
...
def NUM_REP_IND_1(p):
    ...
    with p.have(
        "inner: !P. P IND_1 /\\ (!i:ind. P i ==> P (IND_SUC i)) ==> P IND_1"
    ).proof():
        p.fix("P")
...
def NUM_REP_IND_SUC_CLOSED(p):
    p.goal("!i:ind. NUM_REP i ==> NUM_REP (IND_SUC i)", types={"P": _P_ind_ty})
    p.fix("i")
    p.assume("h_NRi: NUM_REP i")
    p.have(
        "eq_i: NUM_REP i = (!P. P IND_1 /\\ (!j:ind. P j ==> P (IND_SUC j)) ==> P i)"
    ).by_thm(_NUM_REP_unfold(_i_ind))
    p.have(
        "h_NRi_unfold: !P. P IND_1 /\\ (!j:ind. P j ==> P (IND_SUC j)) ==> P i"
    ).by_eq_mp(p.fact("eq_i"), "h_NRi")
    p.have(
        "eq_si: NUM_REP (IND_SUC i) = "
        "(!P. P IND_1 /\\ (!j:ind. P j ==> P (IND_SUC j)) ==> P (IND_SUC i))"
    ).by_thm(_NUM_REP_unfold(mk_comb(IND_SUC, _i_ind)))
    with p.have(
        "inner: !P. P IND_1 /\\ (!j:ind. P j ==> P (IND_SUC j)) ==> P (IND_SUC i)"
    ).proof():
        p.fix("P")
        p.assume("(h_base, h_step): P IND_1 /\\ (!j:ind. P j ==> P (IND_SUC j))")
        p.have("h_NRi_at_P: P IND_1 /\\ (!j:ind. P j ==> P (IND_SUC j)) ==> P i").by(
            SPEC, "P", "h_NRi_unfold"
        )
...
_EXISTS_NUM_REP = EXISTS(
    mk_abs(_a_ind, mk_comb(NUM_REP, _a_ind)), IND_1, NUM_REP_IND_1
)  # |- ?a. (\a. NUM_REP a) a
...
def NUM_REP_dest_num(p):
    ...
    p.fix("a")
    p.have("md_dest: dest_num (mk_num (dest_num a)) = dest_num a").by_thm(
        AP_TERM(dest_num, MK_DEST)
    )
    a_kvar = Var("a", num_ty)
    p.have(
...
def AXIOM_3(p):
    ...
    with p.suppose("h: SUC x = 1"):
        ...
        r_var = Var("r", ind_ty)
        NR_si_dx = MP(
            SPEC(mk_comb(dest_num, x), NUM_REP_IND_SUC_CLOSED),
        ...
        with p.calc("h_peel: IND_SUC (dest_num x)") as c:
            c.step("= dest_num (mk_num (IND_SUC (dest_num x)))").by_thm(
                SYM(eq_lhs_peel)
            )
...
def AXIOM_4(p):
    ...
    r_var = Var("r", ind_ty)
    NR_si_dx = MP(
        SPEC(mk_comb(dest_num, x), NUM_REP_IND_SUC_CLOSED), SPEC(x, NUM_REP_dest_num)
    )
    NR_si_dy = MP(
        SPEC(mk_comb(dest_num, y), NUM_REP_IND_SUC_CLOSED), SPEC(y, NUM_REP_dest_num)
    )
    s_dx = mk_comb(IND_SUC, mk_comb(dest_num, x))
    ...
    with p.calc("h_peeled: IND_SUC (dest_num x)") as c:
        c.step("= dest_num (mk_num (IND_SUC (dest_num x)))").by_thm(SYM(eq_dx_peel))
        c.step("= dest_num (mk_num (IND_SUC (dest_num y)))").by_thm(h_dest)
        c.step("= IND_SUC (dest_num y)").by_thm(eq_dy_peel)
    oo = SPEC(mk_comb(dest_num, y), SPEC(mk_comb(dest_num, x), _ONE_ONE_IND_SUC_unfold))
...
@proof
def INDUCTION(p):
    p.goal("!P. P 1 /\\ (!x. P x ==> P (SUC x)) ==> !x. P x", types={"P": _P_num_ty})
    p.fix("P")
    ...
    with p.have("Q_1: Q IND_1").proof():
        p.have("P_mk_ind1: P (mk_num IND_1)").by_eq_mp(AP_TERM(P, ONE_DEF), "h_base")
        p.thus("Q IND_1").by(CONJ, NUM_REP_IND_1, "P_mk_ind1")

    # ----- !i. Q i ==> Q (IND_SUC i). -----
    with p.have("Q_step: !i:ind. Q i ==> Q (IND_SUC i)").proof():
        p.fix("i")
        p.assume("(NR_i, P_mki): Q i")
        p.have("NR_si: NUM_REP (IND_SUC i)").by_match(NUM_REP_IND_SUC_CLOSED, "NR_i")
        di_eq_i = EQ_MP(INST([(_i_ind, Var("r", ind_ty))], DEST_MK), p.fact("NR_i"))
        SUC_mki_eq_mk_si = TRANS(
            p.unfold(SUC_DEF, mk_comb(mk_num, _i_ind)),
            AP_TERM(mk_num, AP_TERM(IND_SUC, di_eq_i)),
        )
        P_SUC_mki = MP(SPEC(mk_comb(mk_num, _i_ind), p.fact("h_step")), p.fact("P_mki"))
        p.have("P_mk_si: P (mk_num (IND_SUC i))").by_eq_mp(
            AP_TERM(P, SUC_mki_eq_mk_si), P_SUC_mki
        )
        p.thus("Q (IND_SUC i)").by(CONJ, "NR_si", "P_mk_si")

    # ----- For any x, NUM_REP at Q gives Q (dest_num x); peel via MK_DEST. -----
    with p.thus("!x. P x").proof():
        p.fix("x")
        NR_da_unfold = EQ_MP(
            _NUM_REP_unfold(mk_comb(dest_num, x)), SPEC(x, NUM_REP_dest_num)
        )
        p.have("Q_da: Q (dest_num x)").by(
            NR_da_unfold, "Q", CONJ(p.fact("Q_1"), p.fact("Q_step"))
        )
        p.split("Q_da", "(_NR_dx, P_mk_dx)")
        p.thus("P x").by_eq_mp(
            AP_TERM(P, INST([(x, Var("a", num_ty))], MK_DEST)), "P_mk_dx"
        )
...
def R_AT_1(p):
    p.goal(
        "!Q. (Q 1 c /\\ (!k a. Q k a ==> Q (SUC k) (h k a))) ==> Q 1 c", types=_R_TYPES
    )
    p.fix("Q")
...
def R_STEP(p):
    ...
    with p.thus("R c h (SUC n) (h n m)").proof():
        ...
        p.have("step_at: Q n m ==> Q (SUC n) (h n m)").by_thm(
            SPEC(_m, SPEC(_n, p.fact("h_close")))
        )
...
def _mk_unique_at(n_term):
    ...
    unique = mk_forall(
        _m1, mk_forall(_m2, mk_imp(mk_and(R_n_m1, R_n_m2), mk_eq(_m1, _m2)))
    )
...
def R_UNIQUE_BASE(p):
    ...
    p.have("Qp_closure: Qp 1 c /\\ (!k a. Qp k a ==> Qp (SUC k) (h k a))").by(
        CONJ, "Qp_1_c", "Qp_close"
    )
    # Uniqueness.
    with p.have("unique: !m1 m2. R c h 1 m1 /\\ R c h 1 m2 ==> m1 = m2").proof():
        p.fix("m1 m2")
        p.assume("(h_R_m1, h_R_m2): R c h 1 m1 /\\ R c h 1 m2")
        p.have("Qp_1_m1: Qp 1 m1").by("h_R_m1", "Qp", "Qp_closure")
        p.have("Qp_1_m2: Qp 1 m2").by("h_R_m2", "Qp", "Qp_closure")
        p.have("m1_eq_c: m1 = c").by("Qp_1_m1", REFL(ONE))
        p.have("m2_eq_c: m2 = c").by("Qp_1_m2", REFL(ONE))
        p.thus("m1 = m2").by_rewrite(["m1_eq_c", "m2_eq_c"])
    p.thus("(?m. R c h 1 m) /\\ (!m1 m2. R c h 1 m1 /\\ R c h 1 m2 ==> m1 = m2)").by(
        CONJ, "exist", "unique"
    )
...
def R_UNIQUE_STEP(p):
    ...
    p.fix("n")
    p.assume(
        "(IH_exist, IH_unique): (?m. R c h n m) /\\ "
        "(!m1 m2. R c h n m1 /\\ R c h n m2 ==> m1 = m2)"
    )
    ...
    p.have("R_sn_hn: R c h (SUC n) (h n m_n)").by_match(R_STEP, "m_n_eq")
    p.have("exist_sn: ?m. R c h (SUC n) m").by_witness("h n m_n", "R_sn_hn")
    # Uniqueness at SUC n via Qp.
    p.let("Qp(k, a) := R c h k a /\\ (k = SUC n ==> a = h n m_n)", types=_R_TYPES)
    ...
    with p.have("Qp_1_c: Qp 1 c").proof():
        ...
        p.thus("R c h 1 c /\\ (1 = SUC n ==> c = h n m_n)").by(CONJ, R_AT_1, "vac1")
    # !k a. Qp k a ==> Qp (SUC k) (h k a).
    with p.have("Qp_close: !k a. Qp k a ==> Qp (SUC k) (h k a)").proof():
        p.fix("k a")
        p.assume("(R_k_a, _step_part): Qp k a")
        p.have("R_sk_hka: R c h (SUC k) (h k a)").by_match(R_STEP, "R_k_a")
        with p.have("vac2: SUC k = SUC n ==> h k a = h n m_n").proof():
            ...
            kk = Var("kk", num_ty)
            R_func_a = mk_abs(
                kk, parse("R c h kk a", _env_bindings={**p._scope_env(), "kk": kk})
            )
            ...
            p.have("R_n_a: R c h n a").by_eq_mp(R_k_eq_R_n, "R_k_a")
            m_n_term = mk_comb(
                mk_const("@", [(_A, aty)]),
                mk_abs(_m, parse("R c h n m", _env_bindings=p._scope_env())),
            )
            p.have("a_eq_mn: a = m_n").by(
                SPEC(m_n_term, SPEC(_a, p.fact("IH_unique"))),
    ...
    with p.have(
        "unique_sn: !m1 m2. R c h (SUC n) m1 /\\ R c h (SUC n) m2 ==> m1 = m2"
    ).proof():
        p.fix("m1 m2")
        p.assume("(h_R_m1, h_R_m2): R c h (SUC n) m1 /\\ R c h (SUC n) m2")
        p.have("Qp_sn_m1: Qp (SUC n) m1").by("h_R_m1", "Qp", "Qp_closure")
        p.have("Qp_sn_m2: Qp (SUC n) m2").by("h_R_m2", "Qp", "Qp_closure")
...
def NUM_RECURSION(p):
    p.let(_R_LET, types=_R_TYPES)
    p.goal("!c h. ?fn. fn 1 = c /\\ (!n. fn (SUC n) = h n (fn n))", types=_R_TYPES)
    p.fix("c h")
    ...
    sel1_eq_c = MP(
        SPEC(_c, SPEC(sel_pred_R_1, unique_1)), CONJ(R_1_at_sel, R_AT_1)
    )  # |- (@m. R c h 1 m) = c

    # Step 2: !n. (@m. R c h (SUC n) m) = h n (@m. R c h n m).
    with p.have("fn_step: !n. (@m. R c h (SUC n) m) = h n (@m. R c h n m)").proof():
        p.fix("n")
        ...
        R_sn_h_n_sel = MP(SPEC(sel_pred_R_n, SPEC(_n, R_STEP)), R_n_at_sel)
        ...
        p.thus("(@m. R c h (SUC n) m) = h n (@m. R c h n m)").by_thm(
            MP(
                SPEC(h_n_sel, SPEC(sel_pred_R_sn, unique_sn)),
    ...
    fn_sn_eq = TRANS_CHAIN([beta_sn, SPEC(_n, fn_step_un), SYM(h_n_fnn_eq)])
    combined = CONJ(fn_1_eq_c, GEN(_n, fn_sn_eq))
    p.thus("?fn. fn 1 = c /\\ (!n. fn (SUC n) = h n (fn n))").by_witness(
        fn_body, combined
    )
...
def define_recursive(name, fn_ty, x_var, c, h, *, infix=None):
    if not isinstance(h, Abs) or not isinstance(h.body, Abs):
        raise HolError("define_recursive: h must be Abs(k, Abs(a, body))")
    ...
    pred_clean = mk_abs(
        fn_var, mk_and(mk_eq(fn_1, c), mk_forall(n_var, mk_eq(fn_sn, step_clean)))
    )
    ...
    h_red_w = INST(
        [(sel_at_raw, fn_var)], h_red
    )  # |- h n (sel_at_raw n) = clean_step[fn:=sel_at_raw]
    clean_at_n = TRANS(
        raw_at_n, h_red_w
    )  # |- sel_at_raw (SUC n) = clean_step[fn:=sel_at_raw]
    ...
    sel_step = CONJUNCT2(
        pc_at_sel_body
    )  # |- !n. (@pred_clean) (SUC n) = clean_step[fn:=@pred_clean]
