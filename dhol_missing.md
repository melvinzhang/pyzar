...
19. **No rank-1 type operators.** Φ-telescopes can bind type *variables* (`Tyvar("A")`, kind `tp`) but not type *constructors* (`F : tp → tp`, `G : tp → tp → tp`). With this extension you could state `map : ∀F:tp→tp. ∀A B. (A → B) → F A → F B` once and instantiate F at `list`, `option`, `tree`, etc. — most of HOL-Omega's useful expressiveness for generic combinators. Strictly weaker than HOL-Omega: F appears only *applied* (as `F[τ]`), no type-level λ, no βη on types, type-equality stays decidable without a type-level evaluator. Implementation: add `TyopVar(name, kind)` to the Φ vocabulary, add a `TyopApp(F, args)` `hol_type` variant (distinct from `Tyapp`, which is for declared type constants), extend `subst_in_type` / `INST_TYPE` to turn `TyopApp("F", (τ,))` into `Tyapp("list", (τ,), ())` when σ maps F ↦ list. ~150–250 lines. Preserves the 2026 paper's set-theoretic semantics (F is a meta-level placeholder resolved at instantiation, not a value in any model). Sits between item 5 (full higher-kinded dependency, HOL-Omega) and the current rank-1 polymorphism.
