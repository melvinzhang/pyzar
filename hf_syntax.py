...
from data_type import (
    ...
    _select_collapse_eq,  # noqa: F401  -- compatibility re-export
    _aty_for_select,
    CtorRegistry,
    derive_rec_eq as _derive_rec_eq_generic,
    derive_rec_eq_pw as _derive_rec_eq_pw_generic,
    derive_rec_eq_select as _derive_rec_eq_select_generic,
    derive_rec_eq_select_cond as _derive_rec_eq_select_cond_generic,
)
...
#
# The implementation lives in ``data_type.py``.  ``hf_syntax`` owns only the
# HF constructor registry and thin default-registry wrappers, preserving the
# historical public API while keeping the proof machinery shared with PRST.
# ---------------------------------------------------------------------------
...
def _hf_registry(registry):
    return HF_REGISTRY if registry is None else registry


def derive_rec_eq(REC, target_ctor_name, var_names, *, registry=None):
    return _derive_rec_eq_generic(
        REC, target_ctor_name, var_names, registry=_hf_registry(registry)
    )


def derive_rec_eq_pw(REC, target_ctor_name, var_names, *, registry=None):
    return _derive_rec_eq_pw_generic(
        REC, target_ctor_name, var_names, registry=_hf_registry(registry)
    )
...
def derive_rec_eq_select(
    ...
    return _derive_rec_eq_select_generic(
        REC,
        target_ctor_name,
        var_names,
        extra_arg_vars,
        registry=_hf_registry(registry),
    )
...
def derive_rec_eq_select_cond(
    ...
    return _derive_rec_eq_select_cond_generic(
        REC,
        target_ctor_name,
        var_names,
        extra_arg_vars,
