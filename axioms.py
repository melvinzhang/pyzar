...
# (==>) = \p q. (p /\ q) = p
IMP_DEF = define("==>", bbb_ty, "\\p:bool q:bool. (p /\\ q) = p", infix=(10, "right"))
...
EXISTS_DEF = define(
    "?", "(A -> bool) -> bool", "\\P:A->bool. !q:bool. (!x:A. P x ==> q) ==> q"
)
...
SELECT_AX = new_axiom(
    parse("!P:A->bool. !x:A. P x ==> P (${sel} P)", sel=mk_const("@", []))
)
...
ONE_ONE_DEF = define(
    "ONE_ONE", "(A -> B) -> bool", "\\f:A->B. !x1:A x2:A. f x1 = f x2 ==> x1 = x2"
)

# ONTO = \f:A->B. !y. ?x. y = f x
ONTO_DEF = define("ONTO", "(A -> B) -> bool", "\\f:A->B. !y:B. ?x:A. y = f x")
